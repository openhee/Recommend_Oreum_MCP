...
- `GET /api/oreum/{id}/trail-candidates` semi-automates trail collection: Kakao Maps renders a green trail line but exposes no API to read its geometry, so this queries the OpenStreetMap Overpass API (`fetch_osm_trail_candidates` in `app.py`) for `highway=path|footway|track|bridleway` ways within `OSM_TRAIL_SEARCH_RADIUS_M` (1200m) of the oreum's peak coordinate (or its first entrance if peak is missing), returning up to `OSM_TRAIL_CANDIDATE_LIMIT` (8) candidates sorted by length. This is best-effort and not run automatically for all 328 records — it's a per-oreum button (`#trail-osm-fetch-btn` → `fetchTrailCandidates`) the user triggers manually, since OSM coverage of Jeju hiking paths is partial and a raw import must always be human-reviewed before saving. Candidates are drawn as dashed green preview polylines (`renderTrailCandidateOverlays`, not persisted) for visual comparison against Kakao's own trail rendering; picking "이 후보를 새 등산로로 가져오기" (`importTrailCandidate`) pushes the candidate's points into a new `draftTrails` entry, which then goes through the exact same review/edit/save flow as a manually-drawn trail (undo last point, clear, save) — there is no bespoke mid-path vertex editor, so fixing a bad import currently means trimming from the end or redrawing.
